...
def gen_lin_separable(
    num_points=20, th=np.array([[3], [4]]), th_0=np.array([[0]]), dim=2
):
    """
...
def big_higher_dim_separable():
    X, y = gen_lin_separable(
        num_points=50, dim=6, th=np.array([[3], [4], [2], [1], [0], [3]])
    )
    return X, y


def gen_flipped_lin_separable(
    num_points=20, pflip=0.25, th=np.array([[3], [4]]), th_0=np.array([[0]]), dim=2
):
    """
...
def test_linear_classifier(
    dataFun, learner, learner_params={}, draw=False, refresh=True, pause=False
):
    """
...
expected_perceptron = [
    (np.array([[-9.0], [18.0]]), np.array([[2.0]])),
    (np.array([[0.0], [-3.0]]), np.array([[0.0]])),
]
expected_averaged = [
    (np.array([[-9.0525], [17.5825]]), np.array([[1.9425]])),
...
# ip.src==66.45.255.30 and ip.dst==66.45.255.30
# 66.45.255.30
